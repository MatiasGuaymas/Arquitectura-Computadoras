...
IMAGEN:         .space 1250 #Se necesitan 1250 bytes, 4 bits por pixel, 2500 x 4 = 10000
                ...
                daddi $a0,$0, IMAGEN     #Inicia todos los pixeles en blanco
...
NUMEROS:        #Compruebo si el caracter leido es un numero valido
...
MODOS:          #Cargo valores en los registros $a para usar posteriormente en subrutinas
                dadd $a0,$0,$s5     #A0 = Y
                dadd $a1,$0,$s6     #A1 = X
...
MODO_DESPLAZAMIENTO:    #Si el cursor esta en modo desplazamiento debo imprimir el pixel sobre el que se encuentra el cursor antes de mover este
                ld $t0,PIXEL($0)    #Cargo en t0 el pixel
                sd $t0,0($s1)       #Guardo en DATA el pixel
                daddi $t0,$0,5      #t0=5
                sd $t0,0($s0)       #Imprimo el pixel

GUARDAR_PIXEL:  #Guardo la informacion del pixel al que se va a mover el cursor para no perderlo
...
CARGA:          dsll $t0,$t0,4  #Desplaza hacia la izquierda 4 veces para podes guardar nuevamente el 7
...
CARGA2:         sd $t0,0($t2)   #Guardo la palabra con los codigos del blanco en la memoria
...
PIXEL_ACTUALIZAR:  #A0=Y(0-49) A1=X(0-49) A2=DIR MEMORIA GRAFICA A3=COLOR(1-8)
                ...
                j ACTUALIZAR
SEGUNDA_MITAD:  #Si el color del pixel se encuentra en la segunda mitad del byte 
                ...
                dadd $t0,$t0,$a3    #Cargo el nuevo color 
ACTUALIZAR:     sb $t0,0($v0)       #Guardo el byte en memoria con los cambios realizados
                ld $ra,0($sp)       #Pop ra
